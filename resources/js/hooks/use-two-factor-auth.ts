...
function appendError(errors: string[], message: string): string[] {
...
export const useTwoFactorAuth = (): UseTwoFactorAuthReturn => {
    ...
    const fetchQrCode = useCallback(async (): Promise<void> => {
        try {
            ...
            setErrors((prev) => appendError(prev, 'Failed to fetch QR code'));
            setQrCodeSvg(null);
        }
    }, [submit]);
    ...
    const fetchSetupKey = useCallback(async (): Promise<void> => {
        try {
            ...
            setErrors((prev) =>
                appendError(prev, 'Failed to fetch a setup key'),
            );
            setManualSetupKey(null);
        }
    }, [submit]);
    ...
    const fetchRecoveryCodes = useCallback(async (): Promise<void> => {
        try {
            ...
            setErrors((prev) =>
                appendError(prev, 'Failed to fetch recovery codes'),
            );
            setRecoveryCodesList([]);
        }
    }, [submit]);
